...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    const load = async () => {
      const [profilesRes, coursesRes, paymentsRes, enrollRes] = await Promise.all([
      ...
      setStats({ users: profilesRes.count || 0, courses: courses.length, revenue: totalRevenue });
      ...
      setMonthlyData(Object.entries(mMap).slice(-7).map(([month, enrollments]) => ({ month, enrollments })));
      ...
      payments.filter(p => p.status === 'completed' || p.status === 'paid').forEach(p => {
        ...
        const key = d.toLocaleDateString('en-US', { month: 'short' });
        rMap[key] = (rMap[key] || 0) + Number(p.amount || 0);
      });
      setRevenueData(Object.entries(rMap).slice(-7).map(([month, revenue]) => ({ month, revenue })));
      ...
      <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="stat-card flex items-center gap-3">
            ...
            <div><div className="text-xl font-display font-bold text-foreground">{stats.users.toLocaleString()}</div><div className="text-xs text-muted-foreground">Total Users</div></div>
          </div>
          <div className="stat-card flex items-center gap-3">
            ...
            <div><div className="text-xl font-display font-bold text-foreground">{monthlyData.length > 0 ? `${monthlyData[monthlyData.length - 1]?.enrollments || 0}` : '0'}</div><div className="text-xs text-muted-foreground">Latest Month Enrollments</div></div>
          </div>
          <div className="stat-card flex items-center gap-3">
            ...
            <div><div className="text-xl font-display font-bold text-foreground">₹{stats.revenue > 100000 ? (stats.revenue / 100000).toFixed(1) + 'L' : stats.revenue.toLocaleString()}</div><div className="text-xs text-muted-foreground">Revenue</div></div>
          </div>
          <div className="stat-card flex items-center gap-3">
            ...
            <div><div className="text-xl font-display font-bold text-foreground">{stats.courses}</div><div className="text-xs text-muted-foreground">Active Courses</div></div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-card rounded-xl border border-border p-5">
            ...
            {monthlyData.length === 0 ? <p className="text-sm text-muted-foreground text-center py-10">No enrollment data yet.</p> : (
              <ResponsiveContainer width="100%" height={250}>
