...
serve(async (req) => {
  ...
  try {
    ...
    const priceInPaise = Math.round((course.price || 0) * 100);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/student/payment-success?session_id={CHECKOUT_SESSION_ID}&course_id=${courseId}`,
      cancel_url: `${req.headers.get("origin")}/student/courses`,
      metadata: {
        course_id: courseId,
        user_id: user.id,
      },
